...
> **Reconciled with the shipped code on 2026-07-12.** Where this design first sketched a plan the v1 implementation later refined — the package layout (§5), the go-gh/toolchain pins (D1), the `checks` source (§1.1/§3.1/D15), and the test seams (§5) — the sections below now describe what the code actually does; deliberate departures remain called out under *v1 implementation deviations*. Eight of the nine *Unverified* items were live-checked on 2026-07-12 (marked ✓ in that section).
...
**Live re-checks 2026-07-12** (github.com GraphQL, gh authenticated): items **1**, **2**, **3**, **5**, **6**, **7**, **8**, and **9** are confirmed (✓) — the day's later passes used a probe PR plus a temporary repository ruleset on this repo (prq#10), live observation of populated / queue-enabled public repos, and a real fine-grained PAT. Item 4 (GHES-only) stays open, and item 5's SAML sub-shape remains docs-based; both fail-safe as shipped.

1. **Live merge-queue entry payload** — ✓ verified 2026-07-12 on a populated public queue (erigontech/erigon#22387/#22389): `position` is **1-based** (head entry = 1), `state` was `AWAITING_CHECKS`, and `estimatedTimeToMerge` came back a **non-null Int** (2223 / 4157, seconds), stable across four sweeps ~4 min apart; a queued PR's `mergeStateStatus` is `CLEAN`. prq rendered both PRs `QUEUED` with `queue.position/state/eta_s` end-to-end. Schema-wise `estimatedTimeToMerge` stays a nullable Int (confirmed by introspection), so the null path (`-1` sentinel, `eta_s` omitted) remains and `normalize` keeps handling it.
2. **`NEUTRAL`/`SKIPPED` satisfying required checks** — ✓ verified 2026-07-12 (probe PR prq#10 + a temporary repository ruleset requiring them): a check concluding `SKIPPED` (job-level `if: false`) and one concluding `NEUTRAL` (Checks API) both **satisfy** the required rule — `mergeStateStatus` stayed `UNSTABLE` (an unrelated optional failure), not `BLOCKED`, with both contexts `isRequired: true` — and prq counted them `req_ok` with no blockers. The docs-claimed behavior is real.
3. **Ready-to-enqueue inference** (BLOCKED + all green + `isMergeQueueEnabled` ⇒ CLEAN + `queue.required`) — ✓ verified 2026-07-12 on live queue-enabled repos: apache/datafusion#23336, primer/react#8116, and mastodon/mastodon#39485 were each `mergeStateStatus: BLOCKED` with green required checks and approval, and prq synthesized `CLEAN` + `queue:{required:true}` + the "ready to enqueue" notice for all three. A same-shape PR whose required check had failed (DataDog/integrations-core#24230) correctly stayed `BLOCKED` with the check named — the residual fallback holds. BLOCKED semantics may still drift upstream (cli/cli#10775); the fallback covers that.
4. **`HAS_HOOKS` in the wild** — GHES-only (pre-receive hooks); not reproducible on github.com. Revisit if a GHES user reports.
5. **Fine-grained PAT / SAML error shapes** — ✓ (PAT half) verified 2026-07-12 with a live fine-grained token ("All repositories" access, default metadata-only permissions): GraphQL is supported (single-PR and `--mine` calls succeed on public repos — the historical blanket 401 is gone), and an inaccessible private repo masks the whole `repository` node as a standard `NOT_FOUND` error item — the same envelope classic tokens get — so the classifier needs no fine-grained branch (the real shape is in the fixtures). The SAML sub-shape remains docs-based: no SAML-enforcing org was reachable from this account; the 403-substring match stays as shipped.
6. **`isRequired` under repository rulesets** (vs classic branch protection) and the positive fork case (fork PR with *reported* required checks) — ✓ verified 2026-07-12: on the same PR and commit (prq#10), `isRequired` flipped false→true when a `required_status_checks` ruleset was created (and reverted on delete) — ruleset-required contexts report exactly like classic protection; and a live cross-repository PR (cilium/cilium#47106) reported six contexts `isRequired: true` (both `CheckRun` and legacy `StatusContext`), evaluated against base-repo rules.
