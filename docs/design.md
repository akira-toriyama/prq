...
> **Reconciled with the shipped code on 2026-07-12.** Where this design first sketched a plan the v1 implementation later refined — the package layout (§5), the go-gh/toolchain pins (D1), the `checks` source (§1.1/§3.1/D15), and the test seams (§5) — the sections below now describe what the code actually does; deliberate departures remain called out under *v1 implementation deviations*. Seven of the nine *Unverified* items were live-checked on 2026-07-12 (marked ✓ in that section).
...
**Live re-checks 2026-07-12** (github.com GraphQL, gh authenticated): items **1**, **2**, **3**, **6**, **7**, **8**, and **9** are confirmed (✓) — the day's second pass used a probe PR plus a temporary repository ruleset on this repo (prq#10) and live observation of populated / queue-enabled public repos. Items 4 (GHES-only) and 5 (needs a fine-grained PAT run) stay open; both remain fail-safe as shipped.
